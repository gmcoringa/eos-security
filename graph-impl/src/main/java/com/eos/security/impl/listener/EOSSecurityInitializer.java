...
@Service
public class EOSSecurityInitializer {

	private static final Logger log = LoggerFactory.getLogger(EOSSecurityInitializer.class);
	...
	private EOSUser createUser(String login, String name, EOSUserType type) {
		...
		if (user == null) {
			...
			user = new EOSUser().setFirstName("EOS").setLastName(name).setLogin(login)
					.setPersonalMail(login + "@eossecurity.com").setType(type);
		...
		if (userTenant == null) {
			...
			userTenant = new EOSUser().setLogin(login).setEmail(login + "@eossecurity.com").setNickName(login)
					.setState(EOSState.ACTIVE).setTenantAlias(EOSSystemConstants.ADMIN_TENANT);
