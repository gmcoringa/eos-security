...
public final class ValidationUtils {
	...
	private static String getMessage(String validated, String expected, Object received) {
		return "Expected values for [" + validated + "] are [" + expected + "] but [" + String.valueOf(received)
				+ "] was received";
	}

	static public EOSError maxLength(String fieldName, String value, int max) {
		if (value != null && value.length() > max) {
			String message = getMessage(fieldName, fieldName + " > " + String.valueOf(max),
	...
	static public EOSError validateString(String fieldName, String value, boolean required, int min, int max) {
	...
	static public EOSError requiredNumber(String fieldName, Number value, int min, int max) {
		EOSError error = requiredNotNull(fieldName, value);

		if (error == null && value.doubleValue() < min || value.doubleValue() > max) {
			int code = 0;
			String message = getMessage(String.valueOf(value),
					String.valueOf(min) + " <= " + value + " <= " + String.valueOf(max), String.valueOf(value));
	...
	public static EOSError validateEmail(String fieldName, String email, boolean required) {
		EOSError error = validateString(fieldName, email, required, EntityFieldSizes.EMAIL_MIN_SIZE,
